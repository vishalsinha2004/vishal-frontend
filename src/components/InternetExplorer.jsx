...
const InternetExplorer = ({ initialUrl = "https://www.google.com/webhp?igu=1" }) => {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-os-gray font-sans select-none border border-os-dark-gray shadow-retro-outset">
      
      {/* Top Standard Toolbar */}
      <div className="flex items-center gap-1 p-1 border-b border-os-white shadow-[0_1px_0_#808080]">
        <button className="retro-btn p-1 w-12 flex flex-col items-center justify-center text-[10px] leading-tight" onClick={goBack} disabled={historyIndex === 0}>
          <span className={`text-lg leading-none mb-1 ${historyIndex === 0 ? 'opacity-50' : ''}`}>🔙</span>
          Back
        </button>
        <button className="retro-btn p-1 w-12 flex flex-col items-center justify-center text-[10px] leading-tight" onClick={goForward} disabled={historyIndex === history.length - 1}>
          <span className={`text-lg leading-none mb-1 ${historyIndex === history.length - 1 ? 'opacity-50' : ''}`}>🔜</span>
          Forward
        </button>
        <button className="retro-btn p-1 w-12 flex flex-col items-center justify-center text-[10px] leading-tight" onClick={handleStop}>
          <span className="text-lg leading-none mb-1 text-red-600">❌</span>
          Stop
        </button>
        <button className="retro-btn p-1 w-12 flex flex-col items-center justify-center text-[10px] leading-tight" onClick={handleRefresh}>
          <span className="text-lg leading-none mb-1 text-green-600">🔄</span>
          Refresh
        </button>
        <button className="retro-btn p-1 w-12 flex flex-col items-center justify-center text-[10px] leading-tight" onClick={handleHome}>
          <span className="text-lg leading-none mb-1 text-blue-600">🏠</span>
          Home
        </button>
      </div>

      {/* Address Bar */}
      <div className="flex items-center p-1 border-b border-os-white shadow-[0_1px_0_#808080] gap-2">
      ...
      {/* Links / Favorites Bar */}
      <div className="flex items-center p-1 border-b border-os-dark-gray text-xs bg-os-gray gap-3">
      ...
      {/* Status Bar */}
      <div className="bg-os-gray shadow-retro-inset px-2 py-0.5 text-xs border border-os-dark-gray flex justify-between items-center text-os-text h-6">
